...
LONG MiniDumper::WriteMiniDump(_EXCEPTION_POINTERS *pExceptionInfo )
{
	...
	if (GetModuleFileName(NULL, m_szAppPath, _MAX_PATH))
	{
        ...
        _tcsncat(szDbgHelpPath,_T("DBGHELP.DLL"), _MAX_PATH - 1 - _tcsclen(szDbgHelpPath));
			...
			if (m_bHeadless || (::MessageBox( NULL, szScratch, NULL, MB_YESNO )==IDYES))
			{
				...
				if (hFile!=INVALID_HANDLE_VALUE)
				{
					...
					else
					{
						...
						wsprintf( szScratch, _T("Failed to save dump file to '%s' (error %d)"), m_szDumpPath, (Lint)GetLastError() );
						szResult = szScratch;
					}
					::CloseHandle(hFile);
				}
				else
				{
					...
					wsprintf( szScratch, _T("Failed to create dump file '%s' (error %d)"), m_szDumpPath, (Lint)GetLastError() );
					szResult = szScratch;
				}
			}
		}

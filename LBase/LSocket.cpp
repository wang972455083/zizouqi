...
void LSocket::Connect(boost::asio::ip::tcp::endpoint endpoint)
{
...
void LSocket::Connect(Lstring ip,Lshort port)
{
	boost::asio::ip::tcp::endpoint endpoint(boost::asio::ip::address_v4::from_string(ip), port);
	Connect(endpoint);
}

void LSocket::Connect(Lstring ip,Lshort port, bool& success)
{
	success = false;
...
void LSocket::Start()
{
	...
	LLOG_ERROR("client login ip=%s,port=%d,local_port=%d",p.address().to_v4().to_string().c_str(),m_remotePort,m_Port);
...
void LSocket::OnRecvOneMsgPack(LBuffPtr buff)
{
	...
	m_receiver(buff, shared_from_this());
}
